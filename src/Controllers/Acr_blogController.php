...
use Acr\Acr_blog\Models\Config;
use Acr\Acr_blog\Models\Acr_files_childs;
use Acr\Acr_blog\Models\Acr_files;
use Image;
...
class AcrblogController extends Controller
{
    ...
    function config()
    {
        ...
        return view('Acr_blogv::config', compact('config'));
    }

    function views_image($acr_file_id, $file, $loc = '')
    {
        $loc = empty($loc) ? '/zero/' : '/' . $loc . '/';
        return view('Acr_blogv::views_image', compact('file', 'acr_file_id', 'loc'));
    }
    ...
    function views_galery($acr_file_id)
    {
        ...
        return view('Acr_blogv::views_galery', compact('files', 'acr_file_id'));
    }
    ...
    function views_list($acr_file_id)
    {
        $acr_files_model = new Acr_files_childs();
        $files           = $acr_files_model->where('acr_file_id', $acr_file_id)->get();
        return view('Acr_blogv::views_list', compact('files', 'acr_file_id'));
    }
    ...
    function files_galery($acr_file_id)
    {
        $acr_files_model = new Acr_files_childs();
        $files           = $acr_files_model->where('acr_file_id', $acr_file_id)->get();
        return view('Acr_blogv::files_galery', compact('files', 'acr_file_id'));
    }
    ...
    function files_list($acr_file_id)
    {
        $acr_files_model = new Acr_files_childs();
        $files           = $acr_files_model->where('acr_file_id', $acr_file_id)->get();
        return view('Acr_blogv::files_list', compact('files', 'acr_file_id'));
    }
    ...
    function css()
    {
        return view('Acr_blogv::css')->render();
    }

    function form()
    {
        return view('Acr_blogv::form')->render();
    }

    function form_one()
    {
        return view('Acr_blogv::form_one')->render();
    }

    function js($data)
    {
        return view('Acr_blogv::js', compact('data'))->render();
    }
                ...
                foreach ([$img, $img_med] as $item) {
                    if (!empty($config->watermark)) {
                        ...
                        $item->text($text, $ix, $i_h, function ($font) use ($font_size, $color) {
                            $font->file(base_path('/public_html') . '/acr/blogRighteous.ttf');
                            $font->size($font_size);
                            $font->color($color);
                            $font->align('right');
                            $font->valign('bottom');
